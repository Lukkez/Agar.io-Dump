...
(function(window, $) {
 ...
 var _debugAds = "debugads" in queryString;
 var ABGroups = {};
 ...
 var createABTestGroup = function(abtest_id) {
  if (abtest_id in queryString) {
   return +queryString[abtest_id]
  }
  if (window.localStorage != null) {
   if (window.localStorage[abtest_id] == null) {
    window.localStorage[abtest_id] = 0 + ~~(Math.random() * 100)
   }
   return +window.localStorage[abtest_id]
  }
  return 0
 };
 var dfpInitialized = false;

 function createDFPAds() {
  var dfp = {};
  var dfpBottomInitial = null;
  var dfpBottomLogin = null;
  var dfpStats = null;
  var dfpInitial = null;
  var isInFacebook = window.location.search.indexOf("fb") != -1;
  var onlyShowFirstImpression = isInRubiconGroup("ABGroupRubicon") && !isInFacebook;
  var hasBottomAd = agarAdModule.hasBottomAd;
  if (isInFacebook) {
   ...
   dfpInitial = "/116850162/300x250_init";
   dfpBottomInitial = "/116850162/728x90_init";
   dfp = "/116850162/300x250_login";
   dfpStats = "/116850162/300x250_stats";
   dfpBottomLogin = "/116850162/728x90_login"
  }
  var googletag = window["googletag"];
  googletag.cmd.push(function() {
   googletag["pubads"]()["setTargeting"]("abtest", getABGroup("ABGroupDFP") + "");
   adSlots["ac"].push(googleAdsModule.defineSlot(dfpInitial, [300, 250], "g300x250"));
   if (hasBottomAd) adSlots["ac"].push(googleAdsModule.defineSlot(dfpBottomInitial, [728, 90], "g728x90"));
   if (!onlyShowFirstImpression) {
    adSlots["ab"].push(googleAdsModule.defineSlot(dfpStats, [300, 250], "s300x250"));
    adSlots["aa"].push(googleAdsModule.defineSlot(dfp, [300, 250], "a300x250"));
    if (hasBottomAd) adSlots["aa"].push(googleAdsModule.defineSlot(dfpBottomLogin, [728, 90], "a728x90"))
   }
   googletag.pubads()["enableSingleRequest"]();
   googletag.pubads()["disableInitialLoad"]();
   googletag["enableServices"]();
   dfpInitialized = true;
   if (dfpRefreshQueue != null) {
    console.log("refreshing from queue");
    refreshDFPAd(dfpRefreshQueue)
   }
  })
 }
 var dfpRefreshQueue = null;

 function refreshDFPAd(group) {
  if (!dfpInitialized) {
   console.log("dfpInitialized=" + dfpInitialized);
   console.log("queuing refresh");
   dfpRefreshQueue = group;
   return
  }
  var refreshGroup = [];
  for (var i = 0; i < group.length; i++) {
   if ("type" in group[i] && group[i].type != "dfpads") {
    console.log("trying to refresh a non dfp ad. continue");
    continue
   }
   if (_debugAds) console.log("DFP: refreshing:" + group[i].data["A"]);
   refreshGroup.push(group[i].data)
  }
  if (refreshGroup.length <= 0) return;
  if (!window["googletag"]) return;
  window["googletag"]["cmd"]["push"](function() {
   if (!canRefreshAd) return;
   canRefreshAd = false;
   setTimeout(function() {
    canRefreshAd = true
   }, refreshInterval * 60 * 1e3);
   if (window["googletag"] && window["googletag"]["pubads"] && window["googletag"]["pubads"]()["refresh"]) {
    window["googletag"]["pubads"]()["refresh"](refreshGroup)
   }
  })
 }

 function destroyDFPAd(group) {
  var deleteGroup = [];
  for (var i = 0; i < group.length; i++) {
   if ("type" in group[i] && group[i].type != "dfpads") {
    continue
   }
   if (_debugAds) console.log("DFP: destroying:" + group[i].data["A"]);
   deleteGroup.push(group[i].data)
  }
  if (window["googletag"] && window["googletag"]["pubads"] && window["googletag"]["pubads"]()["clear"]) {
   window["googletag"]["pubads"]()["clear"](deleteGroup)
  }
 }
 var googleAdsModule = function(res) {
  var self = this;
  res.defineSlot = function(unit, dimensions, id) {
   var googletag = window["googletag"];
   var slotInst = googletag["defineSlot"](unit, dimensions, id)["addService"](googletag["pubads"]());
   var slotConfig = {
    type: "dfpads",
    data: slotInst
   };
 ...
 window["googleAdsModule"] = googleAdsModule;

 function createRubiconAds() {
  if (!isInRubiconGroup("ABGroupRubicon")) return;
  var mpuUnit = {
   acct: 13694,
   site: 73068,
   zone: 346604,
  ...
  var isInFacebook = window.location.search.indexOf("fb") != -1;
  if (isInFacebook) return;
  var hasBottomAd = agarAdModule.hasBottomAd;
  adSlots["ab"].push(rubiconAds.defineSlot(mpuUnit, "300x250", "s300x250"));
  adSlots["aa"].push(rubiconAds.defineSlot(mpuUnit, "300x250", "a300x250"));
  if (hasBottomAd) adSlots["aa"].push(rubiconAds.defineSlot(bottomUnit, "728x90", "a728x90"))
 }
 var refreshRubiconAd = function(groups) {
  for (var i = 0; i < groups.length; i++) {
   if ("type" in groups[i] && groups[i].type != "rubicon") {
    continue
   }
   var params = groups[i].data;
   if (_debugAds) console.log("Rubicon: refreshing:" + params.id);
   rubiconAds.runAdSlot(params.id, params.size, params.json)
  }
 };
 var destroyRubiconAd = function(group) {
  for (var i = 0; i < group.length; i++) {
   if ("type" in group[i] && group[i].type != "rubicon") {
    continue
   }
   if (_debugAds) console.log("Rubicon: destroying:" + group[i].data.id, group[i]);
   rubiconAds.deleteAdSlot(group[i].data["id"])
  }
 };
 var rubiconAds = function(res) {
  var self = this;
  var extendObj = function(container, obj) {
   for (var i in obj) {
  ...
  res.runAdSlot = function(id, dim, json) {
   var useSSL = document.location.protocol == "https:" ? "https:" : "http:";
   var size = dim.split("x");
   var fid = id + "-fif";
   var fif = document.createElement("iframe");
   fif.style.cssText = "width: " + size[0] + "px; height: " + size[1] + "px; border: 0; margin: 0; padding: 0; overflow: hidden;";
   fif.setAttribute("scrolling", "no");
   fif.src = "about:blank";
   fif.id = fid;
   document.getElementById(id).appendChild(fif);
   var fifContext = fif.contentWindow ? fif.contentWindow.document : fif.contentDocument.document;
   fifContext.open().write("" + "<html>\n" + "<head>\n" + "<scr" + "ipt type='text/javascript'>" + "inDapIF=true;\n" + "<" + "/scr" + "ipt>\n" + "</head>\n" + "<body style='margin : 0; padding: 0;'>\n" + "<!-- Rubicon Project Smart Tag -->\n" + "<scr" + "ipt type='text/javascript'>\n" + "rp_account = '" + json["acct"] + "';\n" + "rp_site = '" + json["site"] + "';\n" + "rp_zonesize  = '" + json["zone"] + "-" + json["size"] + "';\n" + "rp_adtype = 'js';\n" + "rp_kw = '" + json["kw"] + "';\n" + "rp_visitor = " + json["visitor"] + ";\n" + "rp_inventory = " + json["inventory"] + ";\n" + "<" + "/scr" + "ipt>\n" + "<scr" + "ipt type='text/javascript' src=\"" + useSSL + "//ads.rubiconproject.com/ad/" + json["acct"] + '.js"><' + "/scr" + "ipt>\n" + "</body>\n" + "</html>" + "");
   fifContext.close();
   return fid
  };
  res.defineSlot = function(unit, dimensions, id) {
   ...
   var adJson = extendObj({}, unit);
   adJson["kw"] = window["rpx_params"]["kw"];
   adJson["visitor"] = JSON.stringify(window["rpx_params"]["visitor"]);
   adJson["inventory"] = JSON.stringify(window["rpx_params"]["inventory"]);
   slotConfig.data.json = adJson;
   return slotConfig
  };
  res.refreshAdSlot = function(id, dim, json) {
   var fid = id + "-fif";
   var element = window["jQuery"]("#" + fid);
   if (element != null) {
    element.remove()
   } else {
    console.log("couldn't find element", element, fid)
   }
   this.runAdSlot(id, dim, json)
  };
  res.deleteAdSlot = function(id) {
   var fid = id + "-fif";
   var element = window["jQuery"]("#" + fid);
   if (element != null) {
    element.remove()
  ...
  res.renewAdSlot = function(id, dim, json) {
   this.runAdSlot(id, dim, json)
  };
  return res
 }({});
 window["rubiconAds"] = rubiconAds;
 var hasEngagement = false;
 var supersonicAds = function(ssa) {
  var self = this;
  ssa["BrandConnectDoneEvent"] = function() {
   window["hasEngagement"] = false;
   window["MC"]["checkVideoAds"]()
  };
  ssa["BrandConnectOpenEvent"] = function(offer) {};
  ssa["BrandConnectCloseEvent"] = function(offer) {
   closeVideoContainer();
   SSA_CORE.BrandConnect.close()
  };
  ssa["BrandConnectCompletedEvent"] = function(offer) {};
  ssa["BrandConnectReadyEvent"] = function(engagments) {
   if (typeof engagments !== "undefined" && engagments.length) {
    hasEngagement = true
   } else {
    hasEngagement = false
   }
   window["hasEngagement"] = hasEngagement
  };
  return ssa
 }({});
 window["supersonicAds"] = supersonicAds;

 function openOfferwall() {
  if (SSA_CORE != null) {
   $("#offerwallIframe").attr("src", "http://www.supersonicads.com/delivery/panel.php?applicationKey=" + EnvConfig["supersonic_app_key"] + "&applicationUserId=" + window["ssa_json"]["custom_user_id"] + "&custom_user_id=" + window["ssa_json"]["custom_user_id"] + "&applicationUserIdSignature=optional");
   $("#oferwallContainer").fadeIn(400)
  }
 }
 window["openOfferwall"] = openOfferwall;

 function closeOfferwall() {
  $("#oferwallContainer").fadeOut(400);
  window["MC"]["onOffwerwallClose"]()
 }
 window["closeOfferwall"] = closeOfferwall;

 function openVideoAd() {
  if (SSA_CORE != null && hasEngagement) {
   SSA_CORE.BrandConnect.engage()
  }
 }
 window["openVideoAd"] = openVideoAd;

 function closeVideoContainer() {
  $("#videoContainer").fadeOut(800);
  window["MC"]["onVideoClose"]()
 }
 window["closeVideoContainer"] = closeVideoContainer;

 function adInit() {
  ABGroups["ABGroupDFP"] = createABTestGroup("AB9");
  ABGroups["ABGroupRubicon"] = createABTestGroup("AB12_Rubicon");
  var isInFacebook = window.location.search.indexOf("fb") != -1;
  agarAdModule.hasBottomAd = window["hasBottomAd"];
  if (_debugAds) console.log("Init ads");
  createDFPAds();
  createRubiconAds();
  if (_debugAds) console.log("Ads initted");
  if (_debugAds) console.log("Your group: ", isInRubiconGroup("ABGroupRubicon") ? "rubicon" : "dfp");
  adsPosInit()
 }
 ...
 function adsPosInit() {
  ...
  if (hasBottomAd) googletag.cmd.push(function() {
 ...
 function isInRubiconGroup(group) {
  return !isInFirst(group, 5)
 }

 function isInFirst(group, portion) {
  return getABGroup(group) < portion
 }

 function getABGroup(group) {
  if (group in ABGroups) return ABGroups[group];
  return 0
 }
 window["getABGroup"] = getABGroup;

 function refreshAd(group) {
  refreshDFPAd(group);
  refreshRubiconAd(group)
 }
 window["refreshAd"] = refreshAd;

 function destroyAd(group) {
  destroyDFPAd(group);
  destroyRubiconAd(group)
 }
 window["destroyAd"] = destroyAd;
 var agarAdModule = function() {
  var self = this;
  return {
   hasBottomAd: false
  }
 }();
 window["agarAdModule"] = agarAdModule;
 adInit()
